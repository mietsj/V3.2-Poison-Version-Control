...
transform = torchvision.transforms.Compose([
    ...
    torchvision.transforms.Resize((resize, resize))  # Standard size for spectrogram
])
...
# Create data loaders
train_loader = torch.utils.data.DataLoader(train_speech_commands_padded, batch_size=128, shuffle=True)
validation_loader = torch.utils.data.DataLoader(validation_speech_commands_padded, batch_size=1000)

os.makedirs("../Data", exist_ok=True)
torch.save(train_loader, f"../Data/{filename}-train_loader.pth")
torch.save(validation_loader, f"../Data/{filename}-validation_loader.pth")

joblib.dump(le, f"../Data/{filename}-label_encoder.pkl")
...
class UNetConditional(nn.Module):
    ...
    def pos_encoding(self, t, channels, embed_size):
        ...
        return pos_enc.view(-1, channels, 1, 1).repeat(1, 1, int(embed_size), int(embed_size))

    def label_encoding(self, label, channels, embed_size):
        return self.label_embedding(label)[:, :channels, None, None].repeat(1, 1, int(embed_size), int(embed_size))
    ...
    def forward(self, x, t, label):
        ...
        """
        x1 = self.inc(x)
        x2 = self.down1(x1) + self.pos_encoding(t, 128, resize/2) + self.label_encoding(label, 128, resize/2)
        x3 = self.down2(x2) + self.pos_encoding(t, 256, resize/4) + self.label_encoding(label, 256, resize/4)
        x3 = self.sa1(x3)
        x4 = self.down3(x3) + self.pos_encoding(t, 256, resize/8) + self.label_encoding(label, 256, resize/8)
        x4 = self.sa2(x4)
        x = self.up1(x4, x3) + self.pos_encoding(t, 128, resize/4) + self.label_encoding(label, 128, resize/4)
        x = self.sa3(x)
        x = self.up2(x, x2) + self.pos_encoding(t, 64, resize/2) + self.label_encoding(label, 64, resize/2)
        x = self.up3(x, x1) + self.pos_encoding(t, 64, resize) + self.label_encoding(label, 64, resize)
        output = self.outc(x)
        return output
...
def train_conditional(model, beta, num_epochs=10, lr=1e-3):
    ...
    torch.save(model.state_dict(), f"../Models/{filename}-model.pth")
...
plot_x_hist(x_hist)x_per_class = []
